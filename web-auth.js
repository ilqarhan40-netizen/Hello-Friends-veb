...
window.signOutGoogle = function() {
    ...
    setTimeout(() => { if(lockScreen) lockScreen.classList.remove('opacity-0'); }, 50);
...
window.verifyPhoneCode = function() {
    ...
    if(code.length === 6 && window.confirmationResult) {
        window.confirmationResult.confirm(code).then(() => { alert('Logged in!'); window.closeAuthModal(); }).catch(e => alert('Verification failed: ' + e.message));
    }
};

window.loginWithEmail = function() {
    const emailInput = document.getElementById('auth-email-input'); const passInput = document.getElementById('auth-email-password');
