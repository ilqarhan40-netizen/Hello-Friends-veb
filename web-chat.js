// ==========================================
// HELLO FRIENDS - CHAT, WEBRTC & FIREBASE ENGINE
// Файл: js/web-chat.js
// Назначение: База данных, Сообщения, Звонки, Голосовой ввод
// ==========================================
...
// --- 3. ПЕРЕКЛЮЧЕНИЕ КОМНАТ И TOAST-УВЕДОМЛЕНИЯ ---
...
async function handleNewMessage(snapshot) {
    ...
    // Определяем языки для перевода (зависит от комнаты)
    let targetLangs = [];
...
// --- 5. МИКРОФОНЫ (Голос в текст) ---
document.addEventListener('DOMContentLoaded', () => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    
    // Микрофон главного чата
    ...
    if (mainConfMicBtn && SpeechRecognition) {
        let confRec = new SpeechRecognition();
        confRec.continuous = false; confRec.interimResults = false;
        confRec.onstart = () => { mainConfMicBtn.classList.replace('bg-gray-200', 'bg-red-500'); mainConfMicBtn.classList.replace('dark:bg-gray-700', 'dark:bg-red-600'); mainConfMicBtn.classList.add('animate-pulse'); };
        confRec.onend = () => { mainConfMicBtn.classList.replace('bg-red-500', 'bg-gray-200'); mainConfMicBtn.classList.replace('dark:bg-red-600', 'dark:bg-gray-700'); mainConfMicBtn.classList.remove('animate-pulse'); };
        confRec.onerror = () => { mainConfMicBtn.classList.replace('bg-red-500', 'bg-gray-200'); mainConfMicBtn.classList.replace('dark:bg-red-600', 'dark:bg-gray-700'); mainConfMicBtn.classList.remove('animate-pulse'); };
        confRec.onresult = (e) => { window.sendConfMessage(e.results[0][0].transcript); };
        mainConfMicBtn.addEventListener('click', () => { confRec.lang = 'az-AZ'; try { confRec.start(); } catch(e){} });
    }
...
window.toggleVrCC = function() {
    ...
    if(isVrMarqueeEnabled) {
        if(btn) { btn.classList.remove('bg-gray-600', 'text-gray-300'); btn.classList.add('bg-indigo-600', 'dark:bg-[#00C4CC]', 'text-white', 'dark:text-black'); }
        if(marqueeBox && marqueeBox.innerText !== '') marqueeBox.style.display = 'block';
    } else {
        if(btn) { btn.classList.remove('bg-indigo-600', 'dark:bg-[#00C4CC]', 'text-white', 'dark:text-black'); btn.classList.add('bg-gray-600', 'text-gray-300'); }
        if(marqueeBox) marqueeBox.style.display = 'none';
    }
};
...
window.sendConfMessage = function(voiceText = null) {
    const confInput = document.getElementById('conf-text-input');
    const text = typeof voiceText === 'string' ? voiceText : (confInput ? confInput.value.trim() : '');
    if(!text) return;
...
// Запуск дефолтного чата после загрузки
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => { window.switchWebChat('global'); }, 500);
});
